...
export const getTemperature = (latitude: number, longitude: number) =>
  axios.get<TemperatureResponse>(
    `${TEMPERATURE_API_BASE_URL}?latitude=${latitude}&longitude=${longitude}&${FORECAST_ADDITIONAL_URL_PARAMS}`
  );
