...
export const airQualityIndex = (latitude: number, longitude: number) =>
  axios.get<AirQualityAndReveseGeocodeResponse>(
    `${AIR_QUALITY_IDEX_API_BASE_URL}${latitude};${longitude}/?token=${AIR_QUALITY_INDEX_API_TOKEN}`
  );
